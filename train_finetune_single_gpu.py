...
def main(opt):
    ...
    if len(opt.ocr_model) > 0:
        ocr_model.load_state_dict(torch.load(opt.ocr_model, map_location='cpu'), strict=False)
    ...
    """build trainer"""
    trainer = Trainer(diffusion, unet, vae, criterion, optimizer, train_loader, logs, test_loader, device, ocr_model,
                      ctc_loss)
    trainer.train()
